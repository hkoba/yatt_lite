...
よりテンプレート向きの独自構文 LRXML (Loose but Recursive XML)
を採用しています。
...
L<yatt 宣言|/YATT Declaration> (部品の始まり)

=item C<< &yatt:...; >>

L<Entity 参照|/Entity reference> (変数や関数、メソッドの呼び出し)
...
L<ロジックを直接埋め込みたい時|/Processing instruction>
...
書式は YAML か L<XHF|YATT::Lite::XHF> 形式です。

=item C<.htyattrc.pl>

ディレクトリ毎の L<YATT::Lite> インスタンスで各種ハンドラをオーバロードしたり、
ディレクトリ毎の Entity 関数を定義したい時に使います。


=back

残念ながら、現時点では、C<*.ytmpl>, C<.htyattconfig.yml> と
...
記述するために使われます。以下はEntity 参照式の例です。
...
Entity 参照式の要素の内、 C<:> で始まるものは
(javascript の C<.> 記法や perl の C<< -> >> 記法のように)
続けて書くことが出来ます。
...
C<()>, C<{}>, C<[]> の括弧の中には、上記の C<:> で始まる
L<path 要素|/path element>か、
