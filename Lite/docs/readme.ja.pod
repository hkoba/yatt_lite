...
B<yatt> (Yet Another Template Toolkit) は
Web開発を直接的に支援するテンプレートシステムです。
Pure Perl で実装されており、
サンプルとして L<PSGI> ベースの Web フレームワーク
(L<WebMVC0|YATT::Lite::docs::prog_tutorial/WebMVC0>)
が添付されています。
yatt はプログラマーよりも、非プログラマーである Web デザイナーをいかにエンパワーし、
...
など MVC 型設計をうたう最近の Web 開発フレームワークにおいて、
テンプレートは単にレンダラーとして
Controller の用意したデータを受け取るだけの、
あくまで受動的な部品として扱われることが多いように思います。
ですが、結局 B<Web アプリの最大の任務は HTML を生成すること> です。
...
のに、何時の間に Controller からしかデータを渡してはいけない事になったのでしょう?)
...
ただし、(アクセス許可の確認など) B<ユーザからのリクエストの解析処理> だけは、
各テンプレートファイルに散逸させると後が大変なので、
集中的に書きたいところです。
...
=item * テンプレートの手前に薄い Controller を置き、
そこでリクエスト解析処理を集中管理できるようにする
...
yatt で書かれたテンプレートは HTML/XML に非常に構文が似ているので、
導入が容易です
...
振り回されない、簡潔で安定した URI を提供するためです。
ですが (プログラミング知識の無い) Web デザイナーが
url マッピングを変更できないことのデメリットも無視できません。
むしろ、同じことはテンプレートエンジンの抽象化機能でも実現可能なのでは
ないでしょうか？

サンプル Webフレームワーク
L<WebMVC0|YATT::Lite::docs::prog_tutorial/WebMVC0> では、
拡張子 F<.yatt> を隠しつつ、外部からのリクエストを
直接テンプレートのディレクトリ・ファイル名へとマップする機能を用意しています。

=item 多国語化の支援

メッセージ多国語化のための構文があり、専用の xgettext ユーティリティが付属しています。

(残念ながら、最初に生成した .po ファイルには複数行メッセージの扱いにバグがあります。
...
=head2 NOT for wiki-like public templating.
...
=head2 I'm not yet satisfied with current API.
